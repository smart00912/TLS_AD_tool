...
namespace TLS_AD_tool
{
    public class ADHelpers
    {
        ...
        ///域名
        ///
        private static string DomainName = "tls.ad";
        ///
        /// LDAP 地址
        ///
        //   private static string LDAPDomain = "DC=tls,DC=ad";
        ///
        /// LDAP绑定路径
        ///
        private static string ADPath = "LDAP://cnad01.tls.ad/dc=tls,dc=ad";
        ///
        ///登录帐号
        ///
        private static string ADUser = "local.manager";
        ///
        ///登录密码
        ///
        private static string ADPassword = "V0iE/:0i,";
        ///
        ///扮演类实例
        ///
        ...
        public static DirectoryEntry GetDirectoryEntry(string commonName)
        {
            ...
            deSearch.ClientTimeout = new TimeSpan(600);
            deSearch.ServerTimeLimit = new TimeSpan(600);
        ...
        public static DirectoryEntry CreateNewUser(string ldapDN, string commonName, string sAMAccountName, string password)
        {
            ...
            ADHelpers.EnableUser(commonName);
            ADHelpers.SetPassword(commonName, password);
            deUser.Close();
            return deUser;
        }
        ...
        public static DataTable GetAllUsers(string columnname)
        {

            DataTable dt = new DataTable();
            DirectoryEntry de = GetDirectoryObject();
            DirectorySearcher deSearch = new DirectorySearcher(de, "(&(objectCategory=user)(sAMAccountName=*))");
            deSearch.ClientTimeout = new TimeSpan(0, 1, 0);
            ...
            foreach (SearchResult rs in src)
            {
                if (rs != null)
                {
                    ...
                    dr[columnname] = (rs.GetDirectoryEntry().Properties[columnname].Value == null) ? string.Empty : rs.GetDirectoryEntry().Properties[columnname].Value.ToString();
                    dt.Rows.Add(dr);
                }
            }
        ...
        //下面方法有问题
        public static bool CheckUserRights(string username, string group)
        {
            DirectoryEntry de = GetDirectoryEntryOfGroup(group);
            DirectorySearcher deSearch = new DirectorySearcher(de);
            SearchResultCollection src = deSearch.FindAll();
            foreach (SearchResult rs in src)
            {
                if (rs != null)
                {
                    string guser = (rs.GetDirectoryEntry().Properties["sAMAccountName"].Value == null) ? string.Empty : rs.GetDirectoryEntry().Properties["sAMAccountName"].Value.ToString();
                    if (username == guser)
                    {
                        return true;
                    }
        ...
        ///用户帐号
        ///用户新密码
        public static string SetPasswordByAccount(string sAMAccountName, string newPassword)
        {
            DirectoryEntry de = GetDirectoryEntryByAccount(sAMAccountName);
            // 模拟超级管理员，以达到有权限修改用户密码
            IdentityImpersonation impersonate = new IdentityImpersonation(ADUser, ADPassword, DomainName);
        ...
        {
            impersonate.BeginImpersonate();
            de.Properties["userAccountControl"][0] = ADHelpers.ADS_USER_FLAG_ENUM.ADS_UF_NORMAL_ACCOUNT | ADHelpers.ADS_USER_FLAG_ENUM.ADS_UF_DONT_EXPIRE_PASSWD;
            de.CommitChanges();
            impersonate.StopImpersonate();
            de.Close();
        }
        ...
        {
            impersonate.BeginImpersonate();
            de.Properties["userAccountControl"][0] = ADHelpers.ADS_USER_FLAG_ENUM.ADS_UF_NORMAL_ACCOUNT | ADHelpers.ADS_USER_FLAG_ENUM.ADS_UF_DONT_EXPIRE_PASSWD | ADHelpers.ADS_USER_FLAG_ENUM.ADS_UF_ACCOUNTDISABLE;
            de.CommitChanges();
            impersonate.StopImpersonate();
            de.Close();
        }
        ...
        public static int RemoveComputerFromName(string computerName)
        {
            // DirectoryEntry de = ADHelpers.GetDirectoryEntryByAccount(username, password);
            if (computerName.Length==0||computerName.Contains(" "))
            {
                return 1;
            }
            DirectoryEntry de = ADHelpers.GetDirectoryObject();
            DirectorySearcher dsLookForDomain = new DirectorySearcher(de);
        ...
        public static bool Unlock(string userDn)
        {

            DirectoryEntry uEntry = GetDirectoryEntryByAccount(userDn);
